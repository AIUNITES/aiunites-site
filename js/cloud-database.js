...
const CloudDB = {
  ...
  async testConnection() {
    ...
    try {
      ...
      return Array.isArray(data) 
        ? { success: true, status: 'connected', message: `Connected! Found ${data.length} records`, data }
        : { success: false, status: 'invalid', message: 'Invalid response format' };
    } catch (err) {
  ...
  renderAdminPanel(containerId) {
    ...
    container.innerHTML = `
      <div style="background:#1a1a24;border-radius:12px;padding:20px;border:1px solid rgba(255,255,255,0.1);font-family:system-ui,sans-serif;">
        <h3 style="margin:0 0 15px;color:#fff;display:flex;align-items:center;gap:10px;">
          ...
          <span id="clouddb-status" style="font-size:0.75rem;padding:4px 10px;border-radius:12px;background:${enabled?'rgba(34,197,94,0.2)':'rgba(239,68,68,0.2)'};color:${enabled?'#22c55e':'#ef4444'};">${enabled?'● Online':'○ Offline'}</span>
        </h3>
        <div style="margin-bottom:20px;">
          <label style="display:flex;align-items:center;gap:12px;cursor:pointer;color:#fff;">
            <input type="checkbox" id="clouddb-enabled" ${enabled?'checked':''} style="width:20px;height:20px;accent-color:#8b5cf6;">
            <span>Enable Cloud Sync</span>
          </label>
          <p style="margin:8px 0 0 32px;font-size:0.85rem;color:#a0a0b0;">When enabled, data syncs to shared AIUNITES cloud database</p>
        </div>
        <div style="margin-bottom:20px;">
          <label style="display:block;margin-bottom:8px;color:#a0a0b0;font-size:0.9rem;">Apps Script API URL</label>
          <input type="text" id="clouddb-apiurl" value="${apiUrl}" placeholder="https://script.google.com/macros/s/YOUR_ID/exec" style="width:100%;padding:10px 12px;background:#252535;border:2px solid transparent;border-radius:8px;color:#fff;font-size:0.9rem;">
        </div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
          <button id="clouddb-save" style="padding:10px 20px;background:#8b5cf6;color:#fff;border:none;border-radius:8px;font-weight:600;cursor:pointer;">💾 Save Settings</button>
          <button id="clouddb-test" style="padding:10px 20px;background:#252535;color:#fff;border:none;border-radius:8px;font-weight:600;cursor:pointer;">🔄 Test Connection</button>
    ...
    document.getElementById('clouddb-save').addEventListener('click', () => {
      this.setApiUrl(document.getElementById('clouddb-apiurl').value.trim());
      this.setEnabled(document.getElementById('clouddb-enabled').checked);
    ...
    document.getElementById('clouddb-test').addEventListener('click', async () => {
      const msg = document.getElementById('clouddb-message');
      msg.textContent = '🔄 Testing...'; msg.style.color = '#00d4ff';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (script) CloudDB.init({ siteName: script.dataset.clouddbSite });
});
